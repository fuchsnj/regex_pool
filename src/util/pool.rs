...
/// # Example
///
/// This example shows how to share a pool among callers while safely getting
/// exclusive access to a reusable value.
///
/// ```
/// use regex_pool::util::pool::{Pool, PoolGuard};
///
/// let pool = Pool::<_, _, 8>::new(|| Vec::<u8>::new());
/// let mut scratch = pool.get();
/// scratch.extend_from_slice(b"cached allocation");
/// assert_eq!(b"cached allocation", &scratch[..]);
/// PoolGuard::put(scratch);
/// ```
...
mod inner {
    ...
    // RefUnwindSafe.
    impl<T: UnwindSafe, F: UnwindSafe, const MAX_POOL_STACKS: usize>
        RefUnwindSafe for Pool<T, F, MAX_POOL_STACKS>
    {
    }
    ...
    impl<T, F, const MAX_POOL_STACKS: usize> Pool<T, F, MAX_POOL_STACKS> {
        /// Create a new pool. The given closure is used to create values in
        /// the pool when necessary.
        pub(super) const fn new(create: F) -> Pool<T, F, MAX_POOL_STACKS> {
    ...
    impl<T: core::fmt::Debug, F, const MAX_POOL_STACKS: usize>
        core::fmt::Debug for Pool<T, F, MAX_POOL_STACKS>
    {
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
